...
class GridEncoder(nn.Module):
    def __init__(self, input_dim=3, num_levels=16, level_dim=2,
        ...
        offsets.append(offset)
        offsets = torch.from_numpy(np.array(offsets, dtype=np.int32)).to("cuda")
        self.register_buffer('offsets', offsets)
        idx = torch.empty(offset, dtype=torch.long)
        for i in range(self.num_levels):
            idx[offsets[i]:offsets[i+1]] = i
        self.register_buffer('idx', idx)
        self.register_buffer('grid_sizes', torch.from_numpy(np.array(resolutions, dtype=np.int32)).to("cuda"))

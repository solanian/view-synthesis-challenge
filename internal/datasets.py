...
class ILSHBlender(Dataset):
	"""ILSH Dataset based on blender"""
	def __init__(self,
				split: str,
				data_dir: str,
				config: configs.Config):
	...
	def _load_renderings(self, config):
		...
		"""Load images from disk."""
		if config.render_path:
			raise ValueError('render_path cannot be used for the ILSH dataset.')
		pose_file = os.path.join(self.data_dir, f'transforms_train.json')
		with utils.open_file(pose_file, 'r') as fp:
			meta = json.load(fp)
		images = []
		disp_images = []
		normal_images = []
		cams = []
		for idx, frame in enumerate(tqdm(meta['frames'], desc='Loading ILSH dataset', disable=self.global_rank != 0, leave=False)):
			...
			if self._use_mask:
				mask_file = str(fprefix.replace('images', 'masks') + '.npy')
				if utils.file_exists(mask_file):
					with utils.open_file(mask_file, 'rb') as fp:
						mask = np.load(fp)
						image *= mask
						# im = Image.fromarray(image.astype(np.uint8))
						# im.save("masked_image.jpeg")
...
class LLFF(Dataset):
	...
	def _load_renderings(self, config):
		...
		posefile = os.path.join(self.data_dir, f'poses_bounds.npy')
...
if __name__ == '__main__':
	...
	config.factor = 8
	dataset = LLFF('test', 'data/mip_nerf_360/bicycle', config)
	print(len(dataset))
